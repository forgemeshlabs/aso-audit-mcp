...
export function score(url, origin, checks) {
    ...
    const signals = SIGNALS.map((sig) => {
        ...
        const hits = sig.sources.map((id) => byId.get(id)).filter((c) => !!c);
    ...
    const pillarNames = ["Discoverability", "Identity", "Trust", "Commerce", "Reputation", "Memory"];
    ...
    let tier = null;
    if (asoScore >= 90 && invocable)
        tier = "ASO Certified Autonomous-Commerce-Ready";
    else if (asoScore >= 70 && invocable)
        tier = "ASO Certified Trustable";
    else if (asoScore >= 50 && invocable)
