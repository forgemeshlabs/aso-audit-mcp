...
import { looksHtml, tryJson, sanitizeArtifact } from "./http.js";
...
function describe(r) {
    ...
    if (looksHtml(r))
        return "200 but returned an HTML page (likely SPA/404 fallback), not the expected machine-readable file";
    return "200 but body is not the expected format";
}
function result(def, status, evidence, recommendation, data) {
    return { id: def.id, name: def.name, category: def.category, specUrl: def.specUrl, status, evidence, recommendation, data };
}
function def(id, name, category, description, specUrl) {
...
const D = Object.fromEntries(CHECK_DEFS.map((d) => [d.id, d]));
// ---------------------------------------------------------------------------
// Individual checks
// ---------------------------------------------------------------------------
const robotsTxt = async (ctx) => {
...
const aiBotRules = async (ctx) => {
    ...
    if (found.length >= 3)
        return result(D["ai-bot-rules"], "pass", `Explicit rules for ${found.length} AI crawlers: ${found.slice(0, 8).join(", ")}`);
    if (found.length >= 1)
        return result(D["ai-bot-rules"], "partial", `Rules for only ${found.length} AI crawler(s): ${found.join(", ")}`, "Add explicit groups for the major AI crawlers so your crawl policy is unambiguous.");
...
const sitemap = async (ctx) => {
    ...
    if (m)
        return result(D["sitemap"], "partial", `robots.txt declares ${target} but it returned ${s.status || s.error}`, "Fix the sitemap URL declared in robots.txt.");
...
const linkHeaders = async (ctx) => {
    const r = await ctx.home();
    const link = r.headers["link"];
    if (!link)
        return result(D["link-headers"], "fail", "No Link response header on homepage", 'Expose discovery Link headers, e.g. Link: </llms.txt>; rel="llms-txt", or rel="alternate"; type="text/markdown".');
    const agentRel = /llms|markdown|api-catalog|agent|mcp|describedby/i.test(link);
    if (agentRel)
        return result(D["link-headers"], "pass", `Agent-relevant Link header: ${link.slice(0, 200)}`);
...
const wellKnownAi = async (ctx) => {
    const hit = await ctx.firstHit(["/.well-known/ai", "/.well-known/ai.json"]);
    if (hit) {
        const json = tryJson(hit.res.body);
        if (json)
            return result(D["well-known-ai"], "pass", `${hit.path} serves JSON manifest`, undefined, json);
...
const markdownNegotiation = async (ctx) => {
...
const llmDocs = async (ctx) => {
    const md = await ctx.get("/index.md");
    if (md.status === 200 && !looksHtml(md))
        return result(D["llm-docs"], "pass", "/index.md serves markdown source");
    const full = await ctx.get("/llms-full.txt");
    if (full.status === 200 && !looksHtml(full))
        return result(D["llm-docs"], "pass", "/llms-full.txt provides full LLM-readable documentation");
...
const contentSignals = async (ctx) => {
...
const oauthAs = async (ctx) => {
    const r = await ctx.get("/.well-known/oauth-authorization-server");
    const json = tryJson(r.body);
    if (r.status === 200 && json?.issuer)
        return result(D["oauth-as"], "pass", `OAuth AS metadata, issuer: ${json.issuer}`);
    if (r.status === 200 && json)
        return result(D["oauth-as"], "partial", "Metadata JSON found but missing 'issuer'", "RFC 8414 metadata must include 'issuer'.");
...
const oauthPr = async (ctx) => {
    const r = await ctx.get("/.well-known/oauth-protected-resource");
    const json = tryJson(r.body);
    if (r.status === 200 && json?.resource)
        return result(D["oauth-pr"], "pass", `Protected resource metadata for ${json.resource}`);
    if (r.status === 200 && json)
        return result(D["oauth-pr"], "partial", "JSON found but missing 'resource'", "RFC 9728 metadata must include 'resource'.");
...
const authMd = async (ctx) => {
    const hit = await ctx.firstHit(["/auth.md", "/.well-known/auth.md"]);
    if (hit)
        return result(D["auth-md"], "pass", `${hit.path} published (${hit.res.body.length} bytes)`);
...
const a2aAgentCard = async (ctx) => {
    ...
    if (r.status === 200 && json) {
        ...
        if ("supportedInterfaces" in json && (interfaces.length === 0
...
const mcpServerCard = async (ctx) => {
    const card = await ctx.get("/.well-known/mcp/server-card.json");
    const cj = tryJson(card.body);
    if (card.status === 200 && cj) {
        const name = cj.name ?? cj.serverInfo?.name;
...
const webmcp = async (ctx) => {
    const manifest = await ctx.get("/.well-known/webmcp.json");
    if (manifest.status === 200 && tryJson(manifest.body))
        return result(D["webmcp"], "pass", "/.well-known/webmcp.json published");
    const home = await ctx.home();
    if (/modelcontext|webmcp/i.test(home.body))
        return result(D["webmcp"], "partial", "WebMCP references detected in homepage markup");
...
function x402Descriptions(manifest) {
    const out = [];
    const walk = (node) => {
        ...
        for (const key of ["resources", "accepts", "items", "resource"])
            walk(o[key]);
    };
    walk(manifest);
    return out;
}
const x402 = async (ctx) => {
    ...
    return result(D["x402"], "partial", `x402 manifest only at /.well-known/x402.json — the extensionless /.well-known/x402 is missing.${descNote}`, "Serve the manifest at the extensionless /.well-known/x402 as well (crawlers request it first; an HTML 404 there kills some crawls)." + (overLimit > 0 ? " Also trim every description to ≤500 characters." : ""), jsonManifest);
};
const mpp = async (ctx) => {
    const hit = await ctx.firstHit(["/.well-known/mpp", "/.well-known/mpp.json"]);
    if (hit && tryJson(hit.res.body))
        return result(D["mpp"], "pass", `MPP manifest at ${hit.path}`);
...
const ucp = async (ctx) => {
    ...
    if (hit && tryJson(hit.res.body))
        return result(D["ucp"], "pass", `UCP capability manifest at ${hit.path}`, undefined, tryJson(hit.res.body));
...
const acp = async (ctx) => {
    ...
    if (hit && tryJson(hit.res.body))
        return result(D["acp"], "pass", `ACP discovery manifest at ${hit.path}`);
...
const pricing = async (ctx) => {
    const hit = await ctx.firstHit(["/.well-known/pricing.json", "/pricing.json", "/.well-known/payments"]);
    if (hit && tryJson(hit.res.body))
        return result(D["pricing"], "pass", `Machine-readable pricing at ${hit.path}`);
...
const httpsEnforced = async (ctx) => {
    ...
    if (r.error)
        return result(D["https-enforced"], "pass", "HTTPS origin; plain-HTTP port unreachable (strict)");
    if (r.finalUrl.startsWith("https://"))
        return result(D["https-enforced"], "pass", "HTTP redirects to HTTPS");
    return result(D["https-enforced"], "partial", "HTTP does not redirect to HTTPS", "Redirect all plain-HTTP requests to HTTPS.");
};
const jsonLd = async (ctx) => {
    const home = await ctx.home();
...
const agentFriendlyUx = async (ctx) => {
    const home = await ctx.home();
    ...
    const strengths = [];
    const gaps = [];
    if (semanticActions > 0)
        strengths.push(`${semanticActions} semantic interactive element(s)`);
    else
        gaps.push("no semantic button/link/input elements found");
    if (linkedLabels > 0 || unlabeledInputs === 0)
        strengths.push(linkedLabels > 0 ? `${linkedLabels} label(s) linked with for=` : "no form controls needing labels detected");
    else
        gaps.push("form controls found without linked labels");
    if (hasAria || semanticActions > 0)
        strengths.push("ARIA/semantic action signals present");
    else
        gaps.push("no ARIA/role fallback signals");
    if (hasCursorPointer)
        strengths.push("cursor:pointer action cues present");
    if (divButtons > 0)
        gaps.push(`${divButtons} div role=button fallback(s); prefer native button/a where possible`);
    if (ghostRisk)
        gaps.push("hidden/transparent/overlay styling patterns detected");
    ...
    if (semanticActions > 0 || hasAria || linkedLabels > 0) {
        return result(D["agent-friendly-ux"], "partial", `Some agent-friendly signals found (${strengths.join("; ") || "limited"}), gaps: ${gaps.join("; ")}`, "Follow Google's agent-friendly website guidance: prefer semantic button/a/input elements, connect labels with for=, keep layouts stable, and avoid ghost overlays.");
    }
    return result(D["agent-friendly-ux"], "fail", `No strong browser-agent UX signals. Gaps: ${gaps.join("; ")}`, "Use semantic HTML for actions, label form controls, expose meaningful roles/names/states, and keep critical UI stable for screenshot, DOM, and accessibility-tree agents.");
};
const openapi = async (ctx) => {
    const hit = await ctx.firstHit(["/openapi.json", "/.well-known/openapi.json", "/openapi.yaml", "/openapi.yml", "/swagger.json", "/api/openapi.json"]);
    if (hit) {
        const j = tryJson(hit.res.body);
        if (j && (j.openapi || j.swagger)) {
            const info = j.info && typeof j.info === "object" ? j.info : null;
            const paths = j.paths && typeof j.paths === "object" && !Array.isArray(j.paths) ? j.paths : null;
            const missing = [
            ...
            if (missing.length === 0)
                return result(D["openapi"], "pass", `OpenAPI spec at ${hit.path} with ${Object.keys(paths).length} path(s)`, undefined, j);
...
const agentJson = async (ctx) => {
    const hit = await ctx.firstHit(["/agent.json", "/.well-known/agent.json"]);
    const j = hit ? tryJson(hit.res.body) : null;
    if (hit && j && j.name)
        return result(D["agent-json"], "pass", `agent.json at ${hit.path}: "${j.name}"`, undefined, j);
    if (hit && j)
        return result(D["agent-json"], "partial", `${hit.path} is JSON but lacks a 'name'`, "Include at least name, description, capabilities, and signal locations.", j);
...
const securityTxt = async (ctx) => {
    const r = await ctx.get("/.well-known/security.txt");
    if (r.status === 200 && !looksHtml(r) && /contact\s*:/i.test(r.body))
        return result(D["security-txt"], "pass", "RFC 9116 security.txt published");
...
const statusEndpoint = async (ctx) => {
    const hit = await ctx.firstHit(["/.well-known/status", "/status.json", "/api/status", "/health"]);
    if (hit)
        return result(D["status-endpoint"], "pass", `Operational status at ${hit.path}`);
    const home = await ctx.home();
...
const versioning = async (ctx, prior) => {
    ...
    const versions = manifests
        .map((m) => m.version ?? m.info?.version)
        .filter(Boolean);
    if (versions.length)
        return result(D["versioning"], "pass", `Version fields found in ${versions.length} manifest(s): ${versions.join(", ")}`);
    const changelog = await ctx.firstHit(["/changelog.md", "/CHANGELOG.md", "/changelog"]);
    if (changelog)
        return result(D["versioning"], "partial", `Changelog found at ${changelog.path} but manifests carry no version fields`, "Add 'version' fields to your agent.json / Agent Card / OpenAPI info block.");
...
const identityConsistency = async (ctx, prior) => {
    const names = [];
    for (const c of prior) {
        ...
        if (typeof display === "string" && display.trim())
            names.push({ source: c.id, name: display.trim() });
    }
    const llms = prior.find((c) => c.id === "llms-txt");
    if (llms?.status === "pass" && typeof llms.data === "string") {
        const h1 = llms.data.match(/^#\s+(.+)$/m);
        if (h1)
            names.push({ source: "llms-txt", name: h1[1].trim() });
    }
    if (names.length === 0)
        return result(D["identity-consistency"], "fail", "No named manifests published to compare", "Publish agent.json / Agent Card / llms.txt with a consistent service name.");
    if (names.length === 1)
        return result(D["identity-consistency"], "partial", `Only one named source (${names[0].source}: "${names[0].name}")`, "Publish your identity in at least two signal files (e.g. agent.json + llms.txt) with the same name.");
    ...
    const matches = (n) => {
        const x = norm(n);
        if (x.includes(base) || base.includes(x))
            return true;
        return acronym.length >= 3 && new RegExp(`(^|[^a-z0-9])${acronym}([^a-z0-9]|$)`, "i").test(n);
    };
    const consistent = names.every((n) => matches(n.name));
    if (consistent)
        return result(D["identity-consistency"], "pass", `Consistent identity across ${names.length} sources: "${anchor.name}"`);
...
const signalConsistency = async (ctx, prior) => {
    const ok = (id) => prior.find((c) => c.id === id)?.status === "pass";
    ...
    if (present.length >= 3)
        return result(D["signal-consistency"], "pass", `${present.length} core signals present and coherent: ${present.join(", ")}`);
    if (present.length === 2)
        return result(D["signal-consistency"], "partial", `Only 2 core signals present (${present.join(", ")})`, "Publish at least robots.txt + sitemap + llms.txt + one identity manifest, all pointing at the same canonical origin.");
...
export async function runChecks(ctx, only) {
    ...
    const wantDep = (id) => !only || only.includes(id);
    if (wantDep("versioning"))
        results.push(await versioning(ctx, results));
    if (wantDep("identity-consistency"))
        results.push(await identityConsistency(ctx, results));
    if (wantDep("signal-consistency"))
        results.push(await signalConsistency(ctx, results));
